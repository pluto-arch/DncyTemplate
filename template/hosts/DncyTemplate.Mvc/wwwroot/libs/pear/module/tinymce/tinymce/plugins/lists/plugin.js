...
;(function () {
  ...
  var createSegments = function (scope, hosts, size) {
    var segments = []
    for (var i = 0; i < size; i++) {
      segments.push(createSegment(scope, hosts.listType))
    }
    return segments
  }
  var populateSegments = function (segments, hosts) {
    for (var i = 0; i < segments.length - 1; i++) {
      set(segments[i].item, 'list-style-type', 'none')
    }
    last(segments).each(function (segment) {
      setAll(segment.list, hosts.listAttributes)
      setAll(segment.item, hosts.itemAttributes)
      append$1(segment.item, hosts.content)
    })
  }
  var normalizeSegment = function (segment, hosts) {
    if (name(segment.list) !== hosts.listType) {
      segment.list = mutate(segment.list, hosts.listType)
    }
    setAll(segment.list, hosts.listAttributes)
  }
  ...
  var writeShallow = function (scope, cast, hosts) {
    var newCast = cast.slice(0, hosts.depth)
    last(newCast).each(function (segment) {
      var item = createItem(scope, hosts.itemAttributes, hosts.content)
      appendItem(segment, item)
      normalizeSegment(segment, hosts)
    })
    return newCast
  }
  var writeDeep = function (scope, cast, hosts) {
    var segments = createSegments(scope, hosts, hosts.depth - cast.length)
    joinSegments(segments)
    populateSegments(segments, hosts)
  ...
  var composeList = function (scope, entries) {
    var cast = foldl(
      entries,
      function (cast, hosts) {
        return hosts.depth > cast.length
          ? writeDeep(scope, cast, hosts)
          : writeShallow(scope, cast, hosts)
      },
      []
    )
  ...
  var isIndented = function (hosts) {
    return hosts.depth > 0
  }
  var isSelected = function (hosts) {
    return hosts.isSelected
  }
  ...
  var indentEntry = function (indentation, hosts) {
    switch (indentation) {
      case 'Indent':
        hosts.depth++
        break
  ...
  var cleanListProperties = function (hosts) {
    hosts.listAttributes = filter$1(
      hosts.listAttributes,
      function (_value, key) {
        return key !== 'start'
      }
    )
  ...
  var closestSiblingEntry = function (entries, start) {
    ...
    var matches = function (hosts) {
      return hosts.depth === depth && !hosts.dirty
    }
    var until = function (hosts) {
      return hosts.depth < depth
    }
  ...
  var normalizeEntries = function (entries) {
    each(entries, function (hosts, i) {
      closestSiblingEntry(entries, i).fold(
        function () {
          if (hosts.dirty) {
            cleanListProperties(hosts)
          }
        },
        function (matchingEntry) {
          return cloneListProperties(hosts, matchingEntry)
        }
      )
    })
    return entries
  ...
  var outdentedComposer = function (editor, entries) {
    ...
    return map(normalizedEntries, function (hosts) {
      var content = fromElements(hosts.content)
  ...
  var indentSelectedEntries = function (entries, indentation) {
    each(filter(entries, isSelected), function (hosts) {
      return indentEntry(indentation, hosts)
    })
  }
